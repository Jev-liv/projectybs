<x-layouts.app title="Input Data Destoner">
    ...
    <x-ui.card title="Input Data Destoner">
        ...
        <form action="{{ route('kernel.destoner.store') }}" method="POST" id="destonerForm" class="space-y-6">
            @csrf
            ...
            @foreach($kodeFormGroups as $group)
                <div class="space-y-3 rounded-xl border border-rose-200 bg-rose-50 p-4">
                    <h3 class="text-sm font-semibold uppercase tracking-wide text-rose-900">{{ $group['title'] }}</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($group['items'] as $item)
                            @php $kode = $item['kode']; @endphp
                            <div class="border border-rose-200 rounded-lg p-4 bg-white/85 shadow-sm space-y-3" data-kernel-row>
    ...
    <script>
        ...
        const destonerForm = document.getElementById('destonerForm');
        if (destonerForm) {
            let isConfirmedSubmit = false;
            destonerForm.addEventListener('submit', async function (e) {
